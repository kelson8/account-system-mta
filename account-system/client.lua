...
-- TODO Label these values a bit better
...
buttons = {
  ...
  -- { typ = "guest", x = sw * 0.4 - sw * 0.122 + sw * 0.0366, y = sh * 0.5 - (sh * 0.405) / 2 + sw * 0.0854 + sw * 0.0244 + sw * 0.0122 + sw * 0.0244 + sw * 0.0244 + sw * 0.244 * 0.02,   w = sw * 0.244 * 0.3, h = sw * 0.0244, color = tocolor(0, 0, 0, 160),  text = "Back",     func = "pressBack" },
}
  ...
  for index, login in ipairs(field) do
    if login.typ then
      if (login.typ == "login") then
        ...
        if login.text then
          dxDrawText(login.text, login.x, login.y + login.h * 0.2, login.x + login.w, login.y + login.h,
            tocolor(230, 230, 230), sw / 2000, font, "center")
        end
      end
    end
  end
...
function renderLoginContent()
  for i, ebox in ipairs(editbox) do
    if ebox.typ then
      if (ebox.typ == "login") then
        ...
        if ebox.textup then
          dxDrawText(ebox.textup, ebox.x - ebox.x * 0.12, ebox.y + ebox.h * 0.25 + sy, ebox.x + ebox.w, ebox.y + ebox.h,
            tocolor(230, 230, 230), sw / 2000, font)
        end
        ...
        if ebox.text then
          ...
          if ebox.hide == true then
            dxDrawText(string.rep("•", #text[i], ""), ebox.x + 10, ebox.y + sy, ebox.x + ebox.w, ebox.y + ebox.h + sy,
              tocolor(230, 230, 230), sw / 1600, font, "left", "center", true)
          else
            dxDrawText(table.concat(text[i], ""), ebox.x + 10, ebox.y + sy, ebox.x + ebox.w, ebox.y + ebox.h + sy,
              tocolor(230, 230, 230), sw / 1600, font, "left", "center", true)
          end
        end
        if eboxClickI == i then
      ...
      elseif (ebox.typ == "register") then
        ...
        if ebox.textup then
          dxDrawText(ebox.textup, ebox.x - ebox.x * 0.12, ebox.y + ebox.h * 0.25 + ssy, ebox.x + ebox.w, ebox.y + ebox.h,
            tocolor(230, 230, 230), sw / 2000, font)
        end
        ...
        if ebox.text then
          ebox.text = table.concat(text[i], "")
          if ebox.hide == true then
            dxDrawText(string.rep("•", #text[i], ""), ebox.x + 10, ebox.y + ssy, ebox.x + ebox.w, ebox.y + ebox.h + ssy,
              tocolor(230, 230, 230), sw / 1600, font, "left", "center", true)
          else
            dxDrawText(table.concat(text[i], ""), ebox.x + 10, ebox.y + ssy, ebox.x + ebox.w, ebox.y + ebox.h + ssy,
              tocolor(230, 230, 230), sw / 1600, font, "left", "center", true)
          end
        end
        if eboxClickI == i then
  ...
  for index, cbox in ipairs(checkboxs) do
    ...
    if cbox.text then
      dxDrawText(cbox.text, cbox.x + cbox.wh + 5, cbox.y + sy, cbox.x + cbox.wh * 10, cbox.y + cbox.wh + sy,
        tocolor(230, 230, 230), sw / 2000, font, "left", "center", true)
    end
  end
  for index, but in ipairs(buttons) do
    if but.typ then
      if (but.typ == "login") then
        ...
        if but.text then
          dxDrawText(but.text, but.x, but.y + sy, but.x + but.w, but.y + but.h + sy, tocolor(230, 230, 230), sw / 1600,
            font, "center", "center")
        end
      elseif (but.typ == "register") then
        ...
        if but.text then
          dxDrawText(but.text, but.x, but.y + ssy, but.x + but.w, but.y + but.h + ssy, tocolor(230, 230, 230), sw / 1600,
            font, "center", "center")
        end

      -- New for guest
      -- elseif (but.typ == "guest") then
      --   dxDrawImage(but.x, but.y + ssy, but.w, but.h, "ebox.png", 0, 0, 0, but.color)
      --   if isMouseWithinRangeOf(but.x, but.y + ssy, but.w, but.h) then
      --     but.color = tocolor(0, 0, 0, 200)
      --   else
  ...
  for i, ebox in pairs(editbox) do
    if ebox.typ then
      ...
      elseif (ebox.typ == "register") then
        ...
        ----
        -- New for guest mode login
      -- elseif (ebox.typ == "guest") then
      --   if isMouseWithinRangeOf(ebox.x, ebox.y + ssy, ebox.w, ebox.h) then
      --     outputDebugString("Pressed " .. ebox.textup)
      --     eboxClickI = i
      --     return true
